package org.faang.cv.bot.sbertools.salutespeech.token;
...
@Component
public class AccessTokenSaluteSpeechClient {

    private static final String DEFAULT_SCOPE_SALUTE_SPEECH = "SALUTE_SPEECH_PERS";
    ...
    public AccessTokenSaluteSpeechClient(
            @Value("${access.token.url}") String url,
            @Value("${salutespeech.api.token.clientSecret}") String clientSecret,
            @Value("${salutespeech.api.token.clientId}") String clientId,
            ObjectMapper mapper
    ) {
        this.client = client();
        this.url = url;
    ...
    public Token getToken(String rqUID) {
        ...
        RequestBody formBody = new FormBody.Builder()
                .add("scope", DEFAULT_SCOPE_SALUTE_SPEECH)
                .build();
